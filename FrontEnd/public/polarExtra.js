...
(function(Chart) {
  ...
  Chart.defaults.polarExtra.tooltips.callbacks.title = (tooltipItem, data) => {
    return data.labels[tooltipItem[0].index];
  };
  // Values in tooltip
  Chart.defaults.polarExtra.tooltips.callbacks.beforeLabel = (me, tooltipItem, data) => {
    var label =
  ...
  Chart.defaults.polarExtra.tooltips.callbacks.label = () => {
    return '';
  };
  ...
  Chart.controllers.polarExtra = Chart.controllers.polarArea.extend({
    initialize: function(chart, datasetIndex) {
      ...
      chart.config.data.datasets[datasetIndex].backgroundColor = chart.config.data.datasets[datasetIndex].color.map(c =>
        color(c / chart.config.options.colorBase),
      );

      superClass.initialize.call(this, chart, datasetIndex);
    },
    updateElement: function(arc, start, mode) {
      helpers.extend(arc, {
        draw: () => {
          console.log(this);
          var ctx = this.chart.ctx;
          var vm = this.chart.view;
